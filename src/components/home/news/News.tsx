  ...
  return (
    <section ref={sectionRef} id="news-section" className="py-8 sm:py-12 px-4 overflow-hidden relative">
      <div className="max-w-[900px] mx-auto text-left sm:text-center">
        ...
        {/* --- Container for Cards and Arrows --- */}
        <div className="relative flex items-center justify-center"> {/* Added relative positioning here */}
          {/* Left Arrow (Modified) */}
          {newdData.length > 1 && (
            <motion.div
              initial={{ x: -100, opacity: 0 }}
              animate={isVisible ? { x: 0, opacity: 1 } : {}}
              transition={{ duration: 0.7, ease: [0.25, 0.1, 0.25, 1], delay: 0.2 }}
              // Apply positioning directly to motion.div for clarity
              className="absolute -top-3 md:top-1/2 -translate-y-1/2 left-2 lg:left-[-30px] z-50"
            >
              <div
                className="cursor-pointer rounded-full border-none p-1 sm:p-2 transition-colors" // Added padding & background
                onClick={() => setActiveIndex((prev) => (prev - 1 + newdData.length) % newdData.length)}
              >
                <Image
                  src="/logo/left.svg"
                  ...
                  width={24}
                  height={24}
                  className="text-white w-8 h-8 sm:w-12 sm:h-12" // Adjusted display size
                />
              </div>
            </motion.div>
          )}
            ...
            <div className="relative h-full w-full">
              {newdData?.map((item, index) => {
                ...
                return (
                  <motion.div
                    key={item._id || index}
                    className="absolute left-[50%] top-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-2xl overflow-hidden shadow-lg"
                    variants={cardVariants}
                    initial="hidden"
                    animate={position}
                    onClick={() => {
                    ...
                    style={{
                      ...
                      width: "100%",
                      height: "100%",
                      maxWidth: "700px", // Consistent max width
                      maxHeight: "400px", // Consistent max height
          ...
          {newdData.length > 1 && (
            <motion.div
              ...
              transition={{ duration: 0.7, ease: [0.25, 0.1, 0.25, 1], delay: 0.2 }}
              // Apply positioning directly to motion.div for clarity
              className="absolute -top-3 md:top-1/2 max-md:left-[60px] -translate-y-1/2 right-2 lg:right-[-30px] z-50"
            >
              <div
                className="cursor-pointer rounded-full border-none p-1 sm:p-2 transition-colors" // Added padding & background
                onClick={() => setActiveIndex((prev) => (prev + 1) % newdData.length)}
        ...
        {/* --- End Container for Cards and Arrows --- */}
